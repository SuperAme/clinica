<?php  
    ...
    switch($accion){        
        case 'modificar':
            ...
            break;       
        default:
            $sql = $pdo->prepare("SELECT id,CONCAT(title,'--',servicio) as title,doctor,doctor_asignado,enfermera,start,end,materiales,duracion,estatus,servicio,cubiculo,color from citas WHERE estatus = 'Confirmado' and enfermera like '$nombre'");
            $sql -> execute();
